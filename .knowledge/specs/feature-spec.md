...
> 版本: v20.0 | Sprint 21 | 最後更新: 2026-04-10
...
Sprint 20：啟動體驗改造 — 零參啟動（`codeatlas` 無參數等同 `codeatlas web`）、Server idle/analyzing/ready 三態模式、歡迎頁（WelcomePage + ProjectInput 路徑驗證 + RecentProjects 最近專案 + AI 設定引導）、進度頁（ProgressPage + 四階段進度條 scanning/parsing/building/ai_analyzing + SSE 即時推送 + polling 降級 + 自動跳轉 + 失敗重試）、analysis-runner 管線封裝（onProgress 回調 + AbortSignal）、recent-projects 模組（~/.codeatlas/recent.json + max 10 + CRUD）、`/api/project/*` 6 端點（status/validate/analyze/progress SSE/recent GET+DELETE）、AppStateContext 三態路由（welcome/progress/analysis，無 React Router）、Toolbar「切換專案」按鈕、CLI flag 降級優先級鏈（Web 設定 > CLI flag > .codeatlas.json > env > default）、aiConfiguredByWeb 旗標。按需分析（AIJobManager + POST /api/ai/analyze + GET /api/ai/jobs/:jobId + method/method-group/directory/endpoint 四種 scope）、快取持久化（PersistentAICache + 磁碟儲存 + LRU eviction + contentHash 失效）、移除啟動自動全量掃描、Provider 即時切換（POST /api/ai/configure + .codeatlas.json 持久化）、Prompt 全面中文化（PROMPT_VERSION v16.1 + 6 個 prompt 模板全繁體中文 + REPLY_RULES）、Response Sanitizer（markdown fence strip + zod partial parse + 純文字 fallback）、Claude CLI 穩定化（空輸出/stderr 隔離/Windows path）、三視角按需分析按鈕移至右側明細面板（SF SFAISection scope=directory / LO LOAISection scope=method 單一方法級 / DJ DJAIResultSection scope=endpoint + 5 種狀態 + useSyncExternalStore 共享狀態持久化）、AIResultBlock 共用元件（compact/full + RoleBadge 9色 + DJAIBlocks 四區塊色彩）、控制面板分析操作（✨ 分析全部 + ✨ 分析核心目錄 + 進度條）、Settings onChange 連動（Provider select + ⭐ 推薦 + Toast 三色回饋）、Settings 精簡（移除冗餘「故事視角」「顯示偏好」section）、LO 改進（★ 入口方法排序置頂 + 群組卡片一句話摘要 CATEGORY_SUMMARIES）、AI Metrics（success/fail/cacheHit 計數器 + /api/ai/status 暴露）。
Sprint 21：i18n 國際化（中英雙語）— `Locale = 'en' | 'zh-TW'` 型別、react-i18next 框架（Web ~20 元件 t() 替換、en.json/zh-TW.json ~393 keys、Settings 語言切換即時生效 + localStorage 持久化）、AI Prompt 模板化（6 個 prompt 函式吃 locale 參數、prompt-locale.ts 雙語常數、ai-pipeline + server locale 傳遞）、Wiki 輸出語言（md-renderer LOCALE_STRINGS + `wiki --lang` flag）、CLI i18n（輕量 JSON 載入器 + `--lang` 全域選項 + 25 個 CLI 訊息 key）、locale 傳遞鏈（Web: localStorage > navigator > 'en' → API body → server 補齊；CLI: --lang > .codeatlas.json > env > 'en'）、翻譯完整性自動測試（en/zh-TW key parity）。
