...
```
Phase 1（已完成）  Phase 2（已完成）  Phase 3（已完成）    Phase 4（已完成）                   Phase 5（已完成） Phase 6（已完成）     Phase 7（已完成）  Phase 8        Phase 9      Phase 10
...
模組依賴圖        資料流動視覺化   控制面板+UX        S14: LO AI ✅                      死碼移除                             進度顯示 ✅     Wiki 語言     Demo        Plugin 機制
...
已完成            已完成           真正好用          方法級+AI ✅                      精簡 ✅       多語言+知識化 ✅      Web啟動 ✅      中英切換     對外發佈    日常工作流
```
...
| # | 里程碑 | 目標 | 交付物 | 狀態 |
|---|--------|------|--------|------|
| M20 | 啟動體驗 | Web 驅動啟動 | 歡迎頁 + 進度頁 + 零參 CLI | ✅ |

### Sprint 20：啟動體驗改造（✅ 已完成）

> **完成紀錄**：14 任務完成、125 新測試（2111 total 全通過）、G1 圖稿 9 張截圖、前端嚴格比對截圖、G2+G3+G4 全部 ✅ 通過。Sprint 19 附條件（35 過時測試）已清理。

| # | 功能 | 說明 |
|---|------|------|
| 1 | **零參啟動** | `codeatlas` 不帶路徑直接啟動 server，進入「選擇專案」模式 |
| 2 | **歡迎頁/專案選擇** | Web 首頁：輸入路徑 / 瀏覽最近開啟的專案 |
| 3 | **AI Provider 引導** | 首次啟動引導流程包含 AI Provider 選擇（復用現有 Settings 邏輯） |
| 4 | **全域處理進度** | 掃描→解析→建圖→(可選)AI 分析，每步進度條，SSE 即時推送（polling 備案） |
| 5 | **專案記憶** | 記住最近開啟的專案路徑，下次啟動快速選擇（~/.codeatlas/recent.json） |
| 6 | **CLI flag 降級為可選** | `--ai-provider` / `--port` 等 flag 保留但非必須，Web 端可覆蓋 |
| 7 | **過時測試清理** | Sprint 19 G3 附條件：35 個過時測試全部修正歸零 |
